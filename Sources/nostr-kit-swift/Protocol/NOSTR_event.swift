...
extension NOSTR_event_unsigned {
	/// Recompute the event `id` from its current fields.
	/// Returns the id that a freshly constructed event with these exact fields would have.
	public func recomputeID() throws -> NOSTR_id {
		return try computeEventID(publicKey: publicKey, date: date, tags: tags.array, application: application, kind: kind, content: content)
	}
	
	/// Validate the internal consistency and plausibility of the event's fields.
	///
	/// Returns `false` when any of the following hold:
	/// - The stored `id` does not match the recomputed `sha256` of the serialized fields.
	/// - Any tag has an empty name (tags must have a name of at least one character).
	public func isValid() -> Bool {
		guard let computedID = try? recomputeID(), computedID == id else { return false }
...
extension NOSTR_event_unsigned {
	...
	///
	/// The event should only ever be signed once, once it has been confirmed
	/// to be finalized. To prevent signing a stale or mutated event, the id is
...
extension NOSTR_event_signed {
	...
	///
	/// A relay must verify BOTH that the signature is a valid Ed25519 signature
	/// over the event id AND that the stored id matches the recomputed sha256 of
	/// the event's serialized fields. This method enforces both (plus the
